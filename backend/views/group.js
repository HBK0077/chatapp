...
window.addEventListener("DOMContentLoaded", async(e)=>{
    try{
        ...
        const response = await axios.get("http://54.221.155.184:2000/get-groups",{headers:{"Authorization": token}});
...
create.onclick = async(e)=>{
    try{
        ...
        const token = localStorage.getItem('token');
        const response = await axios.post("http://54.221.155.184:2000/add-group",obj,{headers:{"Authorization": token}});
        //console.log(response);
